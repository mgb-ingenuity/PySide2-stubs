...
def resolve_cst_attr(
    node: Union[cst.Name, cst.Attribute, cst.Subscript, cst.SubscriptElement, cst.Index]
) -> str:
...
def self_test():
    assert (
        resolve_cst_attr(cst.parse_expression("PySide2.QtGui.QCursor"))
    ...
    assert (
        resolve_cst_attr(cst.parse_expression("Union[PySide2.QtGui.QCursor] "))
        == "Union[PySide2.QtGui.QCursor]"
    )
    assert (
        resolve_cst_attr(cst.parse_expression("Union[PySide2.QtGui.QCursor ] "))
        == "Union[PySide2.QtGui.QCursor]"
    ...
    assert (
        resolve_cst_attr(cst.parse_expression("Union[PySide2.QtGui.QCursor, aa.bb ] "))
        == "Union[PySide2.QtGui.QCursor, aa.bb]"
    )
    print("Self-test: OK")
            ...
            if fqn_annotation == self.target_annotation:
                ...
                updated_node = updated_node.with_changes(
                    params=updated_node.params.with_changes(
                        ...
                        + (
                            updated_node.params.params[i].with_changes(
                                annotation=cst.Annotation(
                                    annotation=cst.parse_expression(
                                        self.replacement_annotation
                                    )
                                )
                            ),
                        )
...
def fix_auto_conversion_for_module(
    ...
    transformer = FixAutoConversionTypingTransformer(
        module_name, d, target_annotation, replacement_annotation
    )
