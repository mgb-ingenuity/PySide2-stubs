...
class TypingTransformer(cst.CSTTransformer):
    ...
    def leave_ClassDef(
        ...
        flagType, oneFlagClassName, multiFlagClassName = self.fqn_attr_to_type[
            fqn_class
        ]
        return self.add_missing_methods(
            updated_node, flagType, oneFlagClassName, multiFlagClassName
        )
    ...
    def add_missing_methods(
        ...
        flagType: str,
        oneFlagClassName: str,
        multiFlagClassName: str,
    ) -> Union[
        cst.BaseStatement,
        cst.FlattenSentinel[cst.BaseStatement],
        cst.RemovalSentinel,
        ...
        methods_of_qflag = (
            methods_of_oneFlag if flagType == "OneFlag" else methods_of_multiFlag
        )
        ...
        methods_to_add = tuple(
            ...
            if not len(
                matchers.findall(
                    updated_node.body, matchers.FunctionDef(name=matchers.Name(method))
                )
            )
        )
        ...
        return updated_node.with_changes(
            body=updated_node.body.with_changes(
                ...
                + (methods_to_add[0].with_changes(leading_lines=(cst.EmptyLine(),)),)
                + methods_to_add[1:]
            )
        )
